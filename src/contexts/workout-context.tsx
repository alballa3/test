...
type WorkoutAction =
  ...
  | {
      ...
      payload: { exerciseId: number; setId: number };
    }
...
const initialState: WorkoutFormState = {
  ...
  timer: Date.now(),
  exercises: [],
  created_at: moment.now().toLocaleString(),
};
  ...
  switch (action.type) {
    ...
    case "ADD_EXERCISE": {
      const newId =
        state.exercises.length > 0
          ? Math.max(...state.exercises.map((ex) => ex.id)) + 1
          : 1;
      return {
        ...state,
        exercises: [
    ...
    case "MOVE_EXERCISE": {
      ...
      if (
        ...
        (direction === "down" && index === state.exercises.length - 1)
      ) {
    ...
    case "TOGGLE_EXERCISE_EXPANDED":
      return {
        ...
        exercises: state.exercises.map((ex) =>
          ex.id === action.payload ? { ...ex, isExpanded: !ex.isExpanded } : ex
        ),
      };
    ...
    case "UPDATE_REST_TIME":
      return {
        ...
        exercises: state.exercises.map((ex) =>
          ex.id === action.payload.id
            ? { ...ex, restTime: action.payload.value }
            : ex
        ),
      };
    case "ADD_SET": {
      return {
        ...
        exercises: state.exercises.map((ex) => {
          if (ex.id === action.payload) {
            const newSetId =
              ex.sets.length > 0
                ? Math.max(...ex.sets.map((set) => set.id)) + 1
                : 1;
            ...
            return {
              ...
              sets: [
                ...ex.sets,
                { id: newSetId, weight: newWeight, reps: newReps },
              ],
            };
          }
          return ex;
    ...
    case "DUPLICATE_EXERCISE": {
      ...
      const exerciseToDuplicate = state.exercises.find(
        (ex) => ex.id === exerciseId
      );
