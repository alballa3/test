...
import { Calendar, Target, Dumbbell, Heart, Salad, AlertCircle } from "lucide-react"
  ...
  return (
    <section className="bg-zinc-900/60 backdrop-blur-md rounded-xl border border-zinc-800/50 p-5 shadow-lg">
      ...
      {goals && goals.length > 0 ? (
        ...
        <div className="bg-zinc-800/50 backdrop-blur-sm rounded-lg p-6 border border-zinc-700/30 flex flex-col items-center justify-center text-center">
          <AlertCircle className="h-10 w-10 text-zinc-500 mb-3" />
          <h3 className="text-zinc-300 font-medium mb-1">No Goals Found</h3>
          <p className="text-zinc-500 text-sm">This user hasn't set any public fitness goals yet.</p>
        </div>
      )}
    </section>
  );
