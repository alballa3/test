...
export default function FitnessGoals({ profile }: { profile: Profile }) {
  ...
  const handlesave = async () => {
    console.log(goals);
    // const newGoal = goals.filter((prev) => prev.id === id);

    setIsEditing(false);
  };
  const handleAddGoal = async (newGoal: Goal) => {
    console.log(newGoal);
    const reponse = await client.from("goals").insert(newGoal);
  ...
  const handleDeleteGoal = async (id: string) => {
    ...
    const reponse = await client.from("goals").delete().eq("id", id);
    console.log(reponse);
  };
  ...
  const handleUpdateGoal = async (
    ...
    const newGoal = goals.filter((prev) => prev.id === id);
    console.log(newGoal);
    const reponse = await client.from("goals").update(newGoal[0]).eq("id", id);
    console.log(reponse)
  };
              ...
              >
                <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-2">
                  <div className="flex items-center gap-3 w-full">
                    <div
                      className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full ${getCategoryBgColor(
                        goal.category
                      )} flex items-center justify-center text-lg sm:text-xl shadow-inner flex-shrink-0`}
                    >
        ...
        {goals.length === 0 && (
          ...
          >
            ...
            <p className="text-sm max-w-md px-4">
              Set fitness goals to track your progress and stay motivated on
